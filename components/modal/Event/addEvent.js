...
export default function AddEvent(props) {
  ...
  return (
    <SafeAreaView style={tw`m-auto bg-white h-5/6 rounded-xl w-10/12`}>
        ...
        <InputField label='Date' setText={setDate} />
        
        <InputField label='Start Time' setText={setStartTime} />
        
        <Text>Duration</Text>
        <View style={tw`flex-row`}>
            <InputField noLabel type='number' label='Hours' setText={setHours} />
            <InputField noLabel type='number' label='Minutes' setText={setMinutes} />
        </View> 
        ...
        <Dropdown label='Location Type' selected={locationType} setSelected={setLocationType} data={locationTypes}/>
        <InputField label='Address' setText={setAddress} />
        {/* <View>
            <FileUpload label='upload Image' isImage />
        </View> */}
        
        <CheckButton label='Re-Ocurring?' checked={reOccuring} pressed={()=>setReOccuring(!reOccuring)} />
