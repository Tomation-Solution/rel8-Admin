  ...
  return (
    <View style={tw`m-auto bg-white rounded-xl  w-10/12`}>
        ...
        <Pressable onPress={()=>_pickDocument()} style={[tw`${document ? 'pb-1 pt-3' :'py-5'} px-2 my-4 mx-auto  w-11/12 `,{borderWidth:1,borderColor:'#365C2A', borderStyle: 'dashed',
               ...
               { !props.isImage ?
                <Ionicon name="ios-cloud-upload" style={tw`m-auto`} color={document ? 'purple': 'rgba(0,0,0,0.4)'} size={18}/>
                :<Ionicon name="image" style={tw`m-auto`} color={document ? 'purple': 'rgba(0,0,0,0.4)'} size={18}/>}

                <Text style={tw`text-center text-gray-400`} numberOfLines={1}>
                    { props.document ? props.document.uri:
                        props.label
                     }
                </Text>
                {/* {
